...
for mm = 1:numMC
    ...
    for indxSnrDb = 1:length(snrDbSet)
        ...
        BDHP4(indxSnrDb,mm) = testHbGD4bit(A,chanMat,Nrf,snrLin,K,Ns);
    end
end
%%
figure
...
plot(snrDbSet,mean(BDHP4,2),'b--','LineWidth',2),hold on
grid on
xlabel('SNR (dB)')
ylabel('Sum Rate (bps/Hz)')
title(['M_t = ' num2str(Mt) ', N_{RF} = ' num2str(Nrf) ', K = ' num2str(K) ', N = ' num2str(Nf)]);
legend('BD-OFDM-FD','BD-OFDM-HPC','BD-OFDM-HPC (1-bit)','BD-OFDM-HPC (2-bit)','BD-OFDM-HPC (3-bit)','BD-OFDM-HPC (4-bit)')