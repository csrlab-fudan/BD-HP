...
for mm = 1:numMC
    ...
    for indxUser = 1:length(userSet)
        ...
        BDHP(indxUser,mm) = testHbGD(A,chanMat,Nrf,snrLin,K,Ns);
        Nrf1 = 24;
        B = randn(Mt,Nrf1)+1j*randn(Mt,Nrf1);
        BDHP1(indxUser,mm) = testHbGD(B,chanMat,Nrf1,snrLin,K,Ns);
        Nrf2 = 16;
        C = randn(Mt,Nrf2)+1j*randn(Mt,Nrf2);
        BDHP2(indxUser,mm) = testHbGD(C,chanMat,Nrf2,snrLin,K,Ns);
    end
end
%%
figure
